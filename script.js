        ...
        if (cached && cachedTime) {
            ...
            if (age < 24 * 60 * 60 * 1000) {
                ...
                if (data && data.length > 0) {
                    console.log(`📦 تم تحميل ${data.length} مشروع من cache`);
                    return data;
                }
            }
        }
        ...
        if (cached && cachedTime) {
            ...
            if (age < 24 * 60 * 60 * 1000) {
                ...
                if (data && Object.keys(data).length > 0) {
                    console.log(`📦 تم تحميل بيانات التواصل من cache`);
                    return data;
                }
            }
        }
...
// ==================== دوال حفظ واستعادة مكان التمرير المحسنة ====================
...
function saveScrollPosition() {
    ...
    if (appState.currentPage === 'families') {
        pageType = 'families';
        pageId = appState.currentEmirate;
    } else if (appState.currentPage === 'products') {
        pageType = 'products';
        pageId = appState.currentFamilyId;
    } else if (appState.currentPage === 'product-detail') {
        pageType = 'product';
        pageId = `${appState.currentFamilyId}_${appState.currentProductId}`;
    ...
    console.log(`✅ حفظ التمرير: ${scrollKey} = ${scrollPosition}`);
}
...
async function restoreScrollPosition() {
    ...
    if (savedPosition !== null && parseInt(savedPosition) > 0) {
        console.log(`🔄 استعادة التمرير: ${scrollKey} = ${savedPosition}`);
        ...
        requestAnimationFrame(() => {
            ...
            console.log(`✅ تم استعادة التمرير إلى: ${savedPosition}`);
        });
    }
}
...
function waitForContentToLoad() {
    return new Promise((resolve) => {
        const mainContent = document.querySelector('.families-grid, .products-grid, .product-detail-container, .favorites-content, .offers-content');
        ...
        const observer = new MutationObserver((mutations, obs) => {
            const content = document.querySelector('.families-grid, .products-grid, .product-detail-container, .favorites-content, .offers-content');
...
const translations = {
    ...
    en: {
        ...
        navHome: "Home",
        footerKeywords: "Projects Souq | Home Business Platform in UAE | Productive Families Directory",
        navMarket: "Market",
...
function showPage(pageId) {
    ...
    // ✅ تم إزالة السطر الذي كان يعيد التمرير إلى الأعلى
}
    ...
    popup.innerHTML = `
        ...
        <div class="share-options">
            <div class="share-option" onclick="window.open('https://wa.me/?text=${text} ${encodeURIComponent(url)}', '_blank'); closeSharePopup();">
                <i class="fab fa-whatsapp whatsapp-bg"></i>
                <span>WhatsApp</span>
            </div>
            ...
            <div class="share-option" onclick="copyShareLink('${url}'); closeSharePopup();">
                <i class="fas fa-link copy-link-bg"></i>
...
window.addEventListener('load', async function() {
    ...
    const textElements = {
        ...
        'aboutTitle': 'aboutTitle', 'stat1': 'stat1', 'stat2': 'stat2', 'stat3': 'stat3',
        'stat4': 'stat4', 'badge1': 'badge1', 'badge2': 'badge2', 'badge3': 'badge3',
        'badge4': 'badge4', 'familiesPageTitle': 'familiesPageTitle', 'familiesPageSubtitle': 'familiesPageSubtitle',
        'productsTitle': 'productsTitle', 'favoritesPageTitle': 'favoritesPageTitle',
        'favoritesPageSubtitle': 'favoritesPageSubtitle', 'popupTitle': 'popupTitle', 'step1': 'step1',
        'step2': 'step2', 'step3': 'step3', 'gotItBtn': 'gotItBtn', 'shareTitle': 'shareTitle',
        'copyLinkText': 'copyLinkText', 'closeShareBtn': 'closeShareBtn', 'offersBtnText': 'offersBtnText',
        'offersPageTitle': 'offersPageTitle', 'offersPageSubtitle': 'offersPageSubtitle', 'dealsTitle': 'dealsTitle'
    };
    ...
    // ✅ التعديل المهم: تحميل البيانات بشكل صحيح
    let familiesLoaded = false;
    
    const cachedFamilies = loadCachedFamiliesData();
    if (cachedFamilies && cachedFamilies.length > 0) {
        console.log(`✅ تم تحميل ${cachedFamilies.length} مشروع من cache`);
        window.familiesData = cachedFamilies;
        familiesLoaded = true;
    }
    
    if (!familiesLoaded && typeof familiesData !== 'undefined' && familiesData.length > 0) {
        console.log(`✅ تم تحميل ${familiesData.length} مشروع من data.js`);
    ...
    if (!familiesLoaded) {
        console.error('❌ فشل تحميل بيانات المشاريع!');
    }
...
window.addEventListener('pageshow', function(event) {
    // عندما يتم استعادة الصفحة من bfcache (مثل الضغط على زر الرجوع)
    if (event.persisted) {
        console.log('🔄 تم استعادة الصفحة من bfcache، إعادة تطبيق اللغة...');
        
        // 1. التأكد من أن اللغة المتغيرة متطابقة مع المخزنة
        const savedLang = localStorage.getItem('projectSouqLang');
        if (savedLang && savedLang !== appState.currentLanguage) {
            appState.currentLanguage = savedLang;
            toggleLanguage();
        } else {
            // إعادة تطبيق الترجمة للتأكد
            toggleLanguage();
        }
        
        // 2. إعادة تحميل المحتوى حسب الصفحة الحالية
        if (appState.currentPage === 'families') {
            renderFamilies();
        } else if (appState.currentPage === 'products' && appState.currentFamilyId) {
            showFamilyProducts(appState.currentFamilyId, false);
